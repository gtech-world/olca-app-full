...
class ConflictResolutionMap implements ConflictResolver {
	...
	@Override
	public ConflictResolution getResolvedConflict(ModelRef ref) {
	...
	private static ConflictResolutionMap resolve(Commit commit, Conflicts local, Conflicts workspace) {
		...
		if (solved == null)
			return null;
		workspace.equal.forEach(c -> solved.put(c, ConflictResolution.keep()));
		local.equal.forEach(c -> solved.put(c, ConflictResolution.keep()));
	...
	private record Conflicts(List<TriDiff> equal, List<TriDiff> remaining) {
